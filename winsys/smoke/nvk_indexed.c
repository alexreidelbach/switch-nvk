/*
 * nvk_indexed.c — Roadmap Tier 1.1: indexed drawing on the Switch GM20B, by our
 * own NVK. The same textured 3D CUBE as nvk_scene, but drawn the way every real
 * imported mesh is drawn: a vertex buffer of UNIQUE vertices (24 = 4 per face)
 * plus an INDEX buffer (36 indices) via vkCmdBindIndexBuffer + vkCmdDrawIndexed.
 *
 * Proves the GM20B index-fetch path through our driver for BOTH index types:
 *   - records two command buffers (one binds a UINT16 index buffer, one UINT32),
 *   - submits each once headless and verifies the readback (red bg + white glyphs)
 *     => "L16 OK" / "L32 OK",
 *   - then spins the cube on the TV using the UINT16 buffer (indexed in real time).
 *
 *   A..D  instance / device / queue
 *   T  logo texture (upload) + sampler
 *   U  cube vbuf (24 unique verts) + UINT16/UINT32 index buffers + MVP ubo + descriptor set
 *   Z  depth image
 *   N  3D pipeline (scene.vert/scene.frag, vertex input + depth test)
 *   O  two cmd buffers: clear, bind, bindIndexBuffer, vkCmdDrawIndexed(36), copy
 *   L  submit each (16/32), read back, verify red+white present
 *   P  present on the TV (spin, indexed), until +
 *
 * Loaderless ICD (VK_NO_PROTOTYPES); logs sdmc:/nvk_indexed.log.
 */
#include <stdarg.h>
#include <stdio.h>
#include <stdlib.h>
...
#include "shaders/tri_shaders.h"   /* scene_vert_spv[], scene_frag_spv[] + _sz (reused) */
...
#define NVERTS 24u            /* 4 unique verts per face * 6 faces */
#define NIDX   36u            /* 2 triangles per face * 3 * 6 faces */

static FILE *g_log;
static uint32_t g_shot[SCN_PIXELS];
static uint32_t g_tex[TEX_PIXELS];
static float    g_verts[NVERTS * 5];   /* 24 unique verts * (x,y,z,u,v) */
static uint16_t g_idx16[NIDX];
static uint32_t g_idx32[NIDX];
...
/* Build the cube as UNIQUE vertices + an index list (the way real meshes are
 * stored). 4 verts per face (each face has its own UVs, so corners can't be
 * shared across faces) = 24 verts; 2 triangles per face = 36 indices. The
 * triangles are identical to nvk_scene's non-indexed cube (same winding), so a
 * correct indexed draw must look exactly like the proven scene. */
static void gen_cube(void)
{
   ...
   int v = 0, ii = 0;
   for (int f = 0; f < 6; f++) {
      for (int c = 0; c < 4; c++) {            /* 4 unique verts for this face */
         const float *p = CV[F[f][c]];
         g_verts[v*5+0] = p[0]; g_verts[v*5+1] = p[1]; g_verts[v*5+2] = p[2];
         g_verts[v*5+3] = UV[c][0]; g_verts[v*5+4] = UV[c][1];
         v++;
      }
      for (int t = 0; t < 6; t++) {            /* 6 indices into this face's 4 verts */
         uint32_t idx = (uint32_t)(f * 4 + TRI[t]);
         g_idx16[ii] = (uint16_t)idx;
         g_idx32[ii] = idx;
         ii++;
      }
   }
...
int main(void)
{
   g_log = fopen("sdmc:/nvk_indexed.log", "w");
   if (__nxlink_host.s_addr != 0 && R_SUCCEEDED(socketInitializeDefault()))
      nxlinkStdio();
   LOG("=== NVK indexed draw (Roadmap T1.1: indexed cube, UINT16+UINT32) [BUILD idx1] ===");
   gen_tex();
   gen_cube();
   ...
   setenv("MESA_LOG_FILE", "sdmc:/nvk_indexed_mesa.log", 1);
   ...
   VkApplicationInfo app = { .sType = VK_STRUCTURE_TYPE_APPLICATION_INFO,
      .pApplicationName = "nvk_indexed", .apiVersion = VK_API_VERSION_1_1 };
...
#define GD(n) ((PFN_##n)pGDPA(dev, #n))
   ...
   if (!pGetQueue||!pAlloc||!pMap||!pCreateImg||!pBindImg||!pCreateView||!pCreateSamp||
       ...
       !pCopyBI||!pBeginRP||!pBindPipe||!pBindDS||!pBindVB||!pBindIB||!pDrawIndexed||!pEndRP||
...
#define MK_HOST_BUF(buffer, memout, mapout, sz, usg) do { \
   ...
   /* U: vertex buffer (24 unique cube verts) + index buffers (UINT16 + UINT32) + uniform buffer (MVP). */
   ...
   VkBuffer ib16; VkDeviceMemory ib16mem; void *ip16;
   MK_HOST_BUF(ib16, ib16mem, ip16, sizeof g_idx16, VK_BUFFER_USAGE_INDEX_BUFFER_BIT);
   memcpy(ip16, g_idx16, sizeof g_idx16); armDCacheFlush(ip16, sizeof g_idx16);
   VkBuffer ib32; VkDeviceMemory ib32mem; void *ip32;
   MK_HOST_BUF(ib32, ib32mem, ip32, sizeof g_idx32, VK_BUFFER_USAGE_INDEX_BUFFER_BIT);
   memcpy(ip32, g_idx32, sizeof g_idx32); armDCacheFlush(ip32, sizeof g_idx32);
   ...
   LOG("U cube vbuf (%u unique verts) + idx16/idx32 (%u indices) + MVP ubo ready", NVERTS, NIDX);
   ...
   /* Z: depth image + view (D32 ZETA, tiled block-linear kind 0x7b). */
   ...
   /* N: 3D pipeline (same scene shaders + vertex input). */
   ...
   /* O: record TWO command buffers — identical except for the bound index buffer
    * + type (UINT16 vs UINT32). Each: upload tex, render pass, bind, bindIndexBuffer,
    * vkCmdDrawIndexed(36), copy color->readback. */
   ...
   VkCommandBufferAllocateInfo cbai = { .sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO, .commandPool = pool, .level = VK_COMMAND_BUFFER_LEVEL_PRIMARY, .commandBufferCount = 2 };
   r = pAllocCB(dev, &cbai, cmds); if (r) { LOG("FAIL: allocCB %d", r); goto done; }

   struct { VkBuffer ib; VkIndexType type; const char *name; } icfg[2] = {
      { ib16, VK_INDEX_TYPE_UINT16, "UINT16" },
      { ib32, VK_INDEX_TYPE_UINT32, "UINT32" },
   };
   ...
   for (int k = 0; k < 2; k++) {
      VkCommandBuffer c = cmds[k];
      r = pBegin(c, &cbbi); if (r) { LOG("FAIL: begin[%d] %d", k, r); goto done; }
      ...
      pBarrier(c, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, NULL, 0, NULL, 1, &toDst);
      pCopyBI(c, staging, tex, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &bic);
      VkImageMemoryBarrier toRead = { .sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER, .srcAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT, .dstAccessMask = VK_ACCESS_SHADER_READ_BIT,
         ...
         .srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED, .dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED, .image = tex, .subresourceRange = cr };
      pBarrier(c, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, 0, 0, NULL, 0, NULL, 1, &toRead);

      pBeginRP(c, &rpbi, VK_SUBPASS_CONTENTS_INLINE);
      pBindPipe(c, VK_PIPELINE_BIND_POINT_GRAPHICS, pipe);
      pBindDS(c, VK_PIPELINE_BIND_POINT_GRAPHICS, pl, 0, 1, &dset, 0, NULL);
      VkDeviceSize voff = 0; pBindVB(c, 0, 1, &vbuf, &voff);
      pBindIB(c, icfg[k].ib, 0, icfg[k].type);
      pDrawIndexed(c, NIDX, 1, 0, 0, 0);
      pEndRP(c);
      pCopyIB(c, rt, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, rbuf, 1, &region);
      r = pEnd(c); if (r) { LOG("FAIL: end[%d] %d", k, r); goto done; }
   }
   LOG("O recorded 2 indexed cmd buffers (UINT16 + UINT32)");

   /* L: submit each index-type once, read back, verify the cube rendered. The
    * readback check is the rigorous proof (red bg + white glyphs => indexed draw
    * sampled the cube), but it's NON-FATAL: a submit error aborts, yet a zero
    * readback only logs FAIL and continues — so on real HW the TV always shows the
    * spin even if the headless verify is flaky (and on Eden, whose fake GPU reads
    * back zeros, we still proceed instead of bailing). */
   {
      uint32_t *px = (uint32_t *)cpu;
      int allok = 1;
      for (int k = 0; k < 2; k++) {
         memset(cpu, 0, SCN_BYTES); armDCacheFlush(cpu, SCN_BYTES);
         VkSubmitInfo si = { .sType = VK_STRUCTURE_TYPE_SUBMIT_INFO, .commandBufferCount = 1, .pCommandBuffers = &cmds[k] };
         r = pSubmit(queue, 1, &si, VK_NULL_HANDLE);
         if (r != VK_SUCCESS) { LOG("FAIL L%s: submit %d", icfg[k].name, r); goto done; }
         pWaitIdle(queue);
         ...
         uint32_t ctr = px[(SCN_H/2)*SCN_W + SCN_W/2];
         int ok = (red > 0 && white > 0);
         LOG("L %s: red=%u white=%u center=0x%08x => %s", icfg[k].name, red, white, ctr, ok ? "OK" : "FAIL (no pixels - Eden fake GPU, or check on real HW)");
         allok &= ok;
         if (k == 1) memcpy(g_shot, cpu, SCN_BYTES);  /* keep last for an initial on-screen frame */
      }
      if (allok) LOG("=== INDEXED DRAW PASSED on Tegra (UINT16 + UINT32) ===");
      else       LOG("=== indexed readback empty (expected on Eden; verify by the TV on real HW) ===");
   }

   /* P: REAL-TIME LOOP — spin the cube using the UINT16 indexed command buffer.
    * Re-compute the MVP each frame, re-submit, blit to the vsync'd framebuffer. */
   {
      VkCommandBuffer cmd = cmds[0];  /* UINT16 — the common case for game meshes */
      ...
      uint32_t *px = (uint32_t *)cpu;
      float angle = 0.0f; uint32_t frames = 0;
      uint64_t t0 = armGetSystemTick();
      LOG("P: real-time indexed spin @ vsync; press + to exit");
      while (appletMainLoop()) {
         padUpdate(&pad);
         if (padGetButtonsDown(&pad) & HidNpadButton_Plus) {
            /* Exit instantly, skipping the faulting framebuffer/nv teardown (see
             * nvk_scene): terminate the process -> OS reclaims GPU/display/memory. */
   ...
   if (g_log) { fflush(g_log); fclose(g_log); g_log = NULL; }
   /* Normal exit faults on framebuffer apps (recursive User Break in display/nv
    * teardown); terminate directly. */
   svcExitProcess();
