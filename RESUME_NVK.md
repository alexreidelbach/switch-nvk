...
- **CUBEMAP / `samplerCube` proven** (`nvk_cubemap.c`, BUILD cube1, ver 0.50.0) — a 6-layer `VK_IMAGE_CREATE_CUBE_COMPATIBLE` image + `VK_IMAGE_VIEW_TYPE_CUBE` view + Sascha's **verbatim** `texturecubemap` skybox shaders (`sascha_skybox.{vert,frag}`, MIT). On HW: pipeline=0, 6 faces uploaded (one B→I copy/layer), skybox renders, `distinct face colours visible ≥1/6` (one face fills a ~60° FOV — expected), left-stick rotates, ~30 fps (per-frame 1.5 MB cubemap re-upload in the re-submitted cmd; FPS not a concern). Proves the cube-image tiling/array-layer + `samplerCube` paths.
- **➡️ NEXT: see [`ROADMAP.md`](ROADMAP.md)** — the simple plan to prove the driver is ready for real ports (indexed draws → many-draws+blend → **swapchain/WSI** → mips/formats → render-to-texture → compute → **Dawn-Vulkan bring-up**).
- **DEPTH ✅ RESOLVED (2026-05-27) — ground-truth confirmed, the 3D cube renders WITH a depth buffer.** Two-part fix:
  1. **ROOT CAUSE — `src/vulkan/runtime/vk_image.c:99`**: upstream sets `image->drm_format_mod = DRM_FORMAT_MOD_INVALID` only `#if DETECT_OS_LINUX || DETECT_OS_BSD`. Our **HORIZON** port fell outside that guard → `drm_format_mod` stayed `0` = `DRM_FORMAT_MOD_LINEAR` → NIL forced **every** image LINEAR (`gob=0`, `pte_kind=0`). Colour tolerates linear; the GM20B ZETA can't → GR fault on `NVB197_CLEAR_SURFACE`(Z) (method 0x19d0). **Fix: add `DETECT_OS_HORIZON` to that guard** (folded into the patch). Now NIL tiles all images: depth `pte_kind=ZF32 0x7b`, colour `0xfe`, `gob=1` (block-linear).
  2. **`winsys/drm_shim.c` `vm_bind_op`**: NVK carries the PTE kind in the LOW BYTE of the VM_BIND `op->flags` (not GEM_NEW `tile_flags`, which is 0 for OPTIMAL images). Map with `op->flags & 0xff` (fallback `bo->kind`). The depth ZETA then maps block-linear (kind 0x7b) and the clear works.
  Confirmed on HW: `IN: mod=0xffffffffffffff` + `alloc_va: pte_kind=0x7b gob=1` (depth), VM_BIND `opflags=0x7b`, **no ERRNOTIF**, cube renders. **Fix-once → all 3D-with-depth content works (incl. Dawn-over-Vulkan GC/Wii ports).** Diagnosed by instrumenting NIL (temp prints, removed) — pure code-reading "should be 0x7b" contradicted HW "0" until the `mod=0x0` ground-truth pinned it.
